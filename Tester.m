%function [UTotStats, UseStats, p, FinSprSD, UseGameN] = Tester()
...
for h=1:1:1 %% loop that goes through all teams to collect stats and opp stats
  
    Name = [TeamNames{1}{h},'BBXS2018.mat']; % name of team collecting data for
    ...
    [ExcTest] = load(Name); % this file is saved when importing from excel
        ...
        for i =1:1:length(Snew) % determines average team and opp score based on history before that game
            TotPoints = sum(S(1:i));
            TapeSpread(i) = Spread(i)*(.5 + ((i - 1)/((length(Snew)-1)*2)));
            ...
            if i > 1
                ...
                for vv=1:1:(GameNum(i) - 1)
                
                    TapeTest(vv) = Spread(vv)*(.5 + ((1/(i-1))/2)*(vv-1));
            
                end
                
                TapeAvSpread(i) = mean(TapeTest);
            ...
            UseAvLoc(h,:) = [UseLoc(h) Loc(end:-1:end-1)'];

        %%extra stats for use
        UseScore(h) = S(end);
        LastSpread(h) = mean(Spread(1:end-1));
        %UseAvSpread(h,:) = Spread(end-1:-1:end-3);
        UseLastGame(h) = Spread(end);
        ThePctPaint(h) = mean(PctPaint(1:end-1));
        UsePctPaint(h) = mean(PctPaint(1:end));
        %UseAvPctPaint(h,:) = PctPaint(end-1:-1:end-3);
        ThePctOPaint(h) = mean(PctOPaint(1:end-1));
        UsePctOPaint(h) = mean(PctOPaint(1:end));
        %UseAvPctOPaint(h,:) = PctOPaint(end-1:-1:end-3);
 ...
 %/////////////////// Only use if using for analysis
    ...
    [ExcTest2] = load(Name);
       ...
       % Previous Team Opponents 
        ...
        %////////////////////////////////////////////////
            ...
            TapeSOS(j) = QUseSpread(rr)*(.5 + ((j - 1)/((length(Snew)-1)*2)));
            
            clear -regexp ^QString
  ...
  [TapeSpread' Spread(1:end-1)]
  [TapeAvSpread' avSpread']

%   [TapeSOS' SOSnum'];
  
  clear -regexp ^Loc ^A ^ExcTest ^newStats ^newOppStats ^S  ^Snew ^OS ^Spread ^GameNum ^PctPaint ^PctOPaint ^SOSnum ^TapeSpread ^TapeAvSpread
...
UsePctPaint = UsePctPaint - avOffPaint;
UsePctOPaint = UsePctOPaint - avDefPaint;
ThePctPaint = ThePctPaint - avOffPaint;
ThePctOPaint = ThePctOPaint - avDefPaint;
...
for bb=1:numTeams
   ...
   Pred(bb) = sum((p*D).*[UseLoc(bb) LastSpread(bb) abs(ThePctPaint(bb) + ThePctOPaint(UseOpponent(bb))) abs(ThePctOPaint(bb) + ThePctPaint(UseOpponent(bb))) TheSOS(bb) TheSOS(UseOpponent(bb)) LastSpread(UseOpponent(bb))]'); 
   UsePerformTest(bb) = UseLastGame(bb) - Pred(bb);
   
%     
%     for ff =3:-1:1
%     if (UseGameN(bb) - ff + 1) > 30
%         UseAvGameN(bb) = 30;
%     else
%         UseAvGameN(bb) = UseGameN(bb) - ff + 1;
%     end
%     
%         DAv = [UseAvGameN(bb)^2 UseAvGameN(bb) 1]';
%         PredAv(bb,ff) = sum((p*DAv).*[UseAvLoc(bb,ff) QUseSpread(bb) abs(UseAvPctPaint(bb,ff) + UseAvPctOPaint(UseOpponent(bb,ff))) abs(UseAvPctOPaint(bb,ff) + UseAvPctPaint(UseOpponent(bb))) TotTest2(bb) TotTest2(UseOpponent(bb)) QUseSpread(UseOpponent(bb))]'); 
%         UseAvPerformTest(bb) = UseLastGame(bb) - Pred(bb);
%     end
end
...
Round2 = UsePerformTest*0;
Round2Av = UseAvPerform(1:numTeams)*(2/3);
Round3Av = UseAvPerform(1:numTeams)*(1/3);
Round4Av = Round2;

%//////////////Use if new excel data
%UseStats = [QUseSpread TotTest2' Round2' Round4Av' UsePctPaint' UsePctOPaint'];
UseStats = [QUseSpread TotTest2' UsePerformTest' UsePctPaint' UsePctOPaint'];
